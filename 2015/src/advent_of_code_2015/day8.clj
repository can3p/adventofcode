...
(def input "\"sjdivfriyaaqa\\xd2v\\\"k\\\"mpcu\\\"yyu\\\"en\"
\"vcqc\"
...
(defn str-len [s] (count s))

(defn strip-quotes [s] (str/replace s #"(^\"|\"$)" ""))
(defn replace-escapes [s] (str/replace s #"(\\\\|\\\")" "_"))
(defn replace-hex-values [s] (str/replace s #"\\x[0-9a-z]{2}" "_"))
(defn escape-quotes [s] (str/replace s #"\"" "\\\\\""))
(defn escape-slashes [s] (str/replace s "\\" "\\\\"))
(defn add-quotes [s] (str "\"" s "\""))
...
(defn process-input [input]
  (reduce +
          (map (comp diff str/trim)
               (str/split input #"\n"))))

(defn process-input-2 [input]
  (reduce +
          (map (comp str-len add-quotes str/trim)
               (str/split input #"\n"))))

(defn process-input-3 [input]
  (reduce +
          (map (comp str-len str/trim)
               (str/split input #"\n"))))

(def ans1 (process-input input))
(def ans2 (- (process-input-2 (-> input
                             escape-slashes
                             escape-quotes
                             ))
             (process-input-3 input)))
