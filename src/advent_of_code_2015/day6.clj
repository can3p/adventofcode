...
(def input "turn on 489,959 through 759,964
...
(defn get-grid [size]
  (vec (map (fn [x] identity 0) (range 0 (* size size)))))
...
(defn rectangle [start stop]
  (let [[x1 y1] (parse-coord start)
        [x2 y2] (parse-coord stop)
        xs (range x1 (inc x2))
        ys (map (partial * grid-size) (range y1 (inc y2)))
        ]
    (map (fn [[a b]] (+ a b))
         (cart xs ys))))

(defn apply-change [v start stop func]
  (let [idxs (rectangle start stop)]
    (reduce (fn [acc idx]
              (assoc acc idx (func (get acc idx))))
            v idxs)))

(defn turn-on [v start stop]
  (apply-change v start stop
                (fn [x] 1)))

(defn turn-off [v start stop]
  (apply-change v start stop
                (fn [x] 0)))

(defn toggle [v start stop]
  (apply-change v start stop
                (fn [x] (- 1 x))))

(defn process [v s]
  (let [[_ start stop] (re-find #"(\d+,\d+)[^\d]*(\d+,\d+)" s)]
    (cond
      (.contains s "turn on") (turn-on v start stop)
      (.contains s "turn off") (turn-off v start stop)
      (.contains s "toggle") (toggle v start stop)
      )))
...
(defn turn-on-2 [v start stop]
  (apply-change v start stop
                inc))

(defn turn-off-2 [v start stop]
  (apply-change v start stop
                (fn [x] (max 0 (dec x)))))

(defn toggle-2 [v start stop]
  (apply-change v start stop
                (fn [x] (+ 2 x))))
...
(defn process-2 [v s]
  (let [[_ start stop] (re-find #"(\d+,\d+)[^\d]*(\d+,\d+)" s)]
    (cond
      (.contains s "turn on") (turn-on-2 v start stop)
      (.contains s "turn off") (turn-off-2 v start stop)
      (.contains s "toggle") (toggle-2 v start stop)
      )))
